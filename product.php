...
if (isset($_GET['id'])) {
    ...
    $result = $stmt->execute();
    $product_category = $result->fetchArray(SQLITE3_ASSOC);
    $category_name = $product_category['name'];
    $category_image = $product_category['image'];
    $category_provider = $product_category['provider'];    

    $stmt = $db->prepare("SELECT * FROM products WHERE category_id=:category_id LIMIT :offset, :items_per_page");
    $stmt->bindValue(':category_id', $category_id, SQLITE3_TEXT);
    $stmt->bindValue(':offset', $offset, SQLITE3_TEXT);
    $stmt->bindValue(':items_per_page', $items_per_page, SQLITE3_TEXT);
    $result = $stmt->execute();
    if (!$result) {
        die("Failed to fetch products: " . $db->lastErrorMsg());
    }
...
<html>
<head>
    <title><?php echo strtoupper($settings['sitename']); ?>: Products</title>
    <meta charset="UTF-8">
...
<body class="w3-content" style="max-width:1200px">
        ...
        <div class="w3-main" style="margin-left:250px">
            ...
            <header class="w3-container w3-xlarge">
                <p class="w3-left"><?php echo ucfirst(htmlentities($category_name)); ?></p>
                <p class="w3-right">
            ...
            <div class="w3-display-container w3-container">
                <img src="<?php echo htmlentities($category_image); ?>" alt="Jeans" style="width:100%">
            ...
            <div class="w3-container w3-text-grey" id="provider">
                <p>
                <?php if (!empty($category_provider)) : ?>
                    Merchandise provided by <b><?php echo htmlentities($category_provider); ?></b>
                <?php endif; ?>
                </p>
            ...
            <!-- Product grid -->
            <?php $total_pages = $total_products / $settings['items_per_page']; ?>
            <div class="w3-row w3-grayscale">
                ...
                <div class="w3-bar w3-center w3-margin-top">
                    ...
                    <?php if ($current_page > 1): ?>
                        <a href="?id=<?php echo $category_id; ?>&page=<?php echo $current_page - 1; ?>" class="w3-button w3-light-grey">&laquo; Previous</a>
                    <?php endif; ?>

                    <!-- Page Numbers -->
                    <?php for ($page = 1; $page <= $total_pages; $page++): ?>
                        <a href="?id=<?php echo $category_id; ?>&page=<?php echo $page; ?>" 
                    ...
                    <?php if ($current_page < $total_pages): ?>
                        <a href="?id=<?php echo $category_id; ?>&page=<?php echo $current_page + 1; ?>" class="w3-button w3-light-grey">Next &raquo;</a>
                    <?php endif; ?>
                </div>
            </div>
